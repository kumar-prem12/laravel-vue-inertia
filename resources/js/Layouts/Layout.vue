    ...
    <header>
      <nav>
        ...
        <div class="space-x-6">
          ...
          <Link :href="route('about')" class="nav-link" :class="{ 'bg-slate-700': $page.component === 'About' }">About
          </Link>
        </div>
        <div v-if="$page.props.auth.user" class="space-x-6 flex">
          <img class="avatar"
            :src="$page.props.auth.user.avatar ? `/storage/${$page.props.auth.user.avatar}` : '/storage/avatars/user-default.png'"
            alt="">
          ...
          <Link :href="route('posts.index')" class="nav-link"
            :class="{ 'bg-slate-700': $page.component === 'Posts/Index' }">Posts</Link>
